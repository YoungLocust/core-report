...
public class CreateExcelReportController {
	...
	public void createExcelReport(ReportRequest report) throws Exception {
		...
		if (report == null || StringUtils.isBlank(taskId) || StringUtils.isBlank(reportName)
				|| StringUtils.isBlank(reportSql) || keys == null || headers == null) {
		...
		new Thread(new Runnable() {
			@Override
			public void run() {
				coreReportService.createReportExcelFile(taskId, reportName.trim() + ".xlsx", sql,
						report.getParams(), keys, headers);
			}
		}).start();
